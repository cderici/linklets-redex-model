...
(define-metafunction Linklets
  ;; return
  [(run-prog ((program (use-linklets) n) Ω ρ σ)) n] ;; number
  [(run-prog ((program (use-linklets) b) Ω ρ σ)) b] ;; boolean
  [(run-prog ((program (use-linklets) (void)) Ω ρ σ)) (void)] ;; void
  ...
  [(run-prog ((program (use-linklets (x L) ...) stuck) Ω ρ σ)) stuck]
